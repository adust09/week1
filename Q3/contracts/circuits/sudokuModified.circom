...
template sudoku() {
    ...
    component puzzleRangeProof = RangeProof(9,9);
    puzzleRangeProof.range[0] <== 0;
    puzzleRangeProof.range[1] <== 9;

    component solutionRangeProof = RangeProof(9,9);
    solutionRangeProof.range[0] <== 0;
    solutionRangeProof.range[1] <== 9;

    for (var i=0; i<9; i++) {
        for (var j=0; j<9; j++) {

            puzzleRangeProof.in[i][j] <== puzzle[i][j];
            solutionRangeProof.in[i][j] <== solution[i][j];
    ...
    assert(puzzleRangeProof.out == 0);
    assert(solutionRangeProof.out == 0);
